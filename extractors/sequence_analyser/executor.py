...
def bulk_convert_mzml(mzml_dir: str, output_dir: str):
    """Bulk converts mzML data into JSON data

    Arguments:
        mzml_dir {str} -- Path to the mzML file directory
        output_dir {str} -- Directory to write the JSON data to
    """

    # Bulk process all mzML files
    mzml.process_multiple_files(mzml_dir, output_dir)
...
def run(ms_json_dir: str, seq_json_dir: str, output_dir, mzml_folder=""):
    ...
    # If folder given, bulk convert
    if mzml_folder:
        bulk_convert_mzml(mzml_folder, ms_json_dir)
